...
@section('content')
    ...
    <form action="{{ route('admin.settings.updateContacts') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="flex justify-between mt-2 p-2 flex-wrap md:flex-no-wrap">
            <h1 class="text-xl">Настройка контента "Контакты"</h1>
        </div>
        <div>
            <div class="p-2">
                <p>Content</p>
                <div id="managerCommentBlock"
                     ...
                     onclick="editReview()">{!! !empty(old('content')) ? old('content') : $contactsBlock->content !!} </div>
                @error('content')
                <p class="text-center text-red-700 text-sm">{{ $message }}</p>
                @enderror

                <textarea id="mytextarea" name="content"
                          class="hidden editor w-full bg-transparent border border-white rounded">{{ !empty(old('description')) ? old('description') : $contactsBlock->content }}</textarea>
            </div>
