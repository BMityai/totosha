...
@section('content')
    ...
    <form action="{{ route('admin.settings.updatePurchaseReturns') }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            <div class="p-2">
                ...
                <div id="managerCommentBlock"
                     ...
                     onclick="editReview()">{!! !empty(old('content')) ? old('content') : $purchaseReturn->content !!} </div>
                @error('content')
                <p class="text-center text-red-700 text-sm">{{ $message }}</p>
                @enderror

                <textarea id="mytextarea" name="content"
                          class="hidden editor w-full bg-transparent border border-white rounded">{{ !empty(old('description')) ? old('description') : $purchaseReturn->content }}</textarea>
            </div>
