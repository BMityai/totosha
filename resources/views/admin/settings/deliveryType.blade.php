...
@section('title', ': ' . $deliveryType->name)

@section('content')


    <form action="{{ route('admin.settings.updateDeliveryType', $deliveryType->code) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="flex justify-between mt-2 p-2 flex-wrap md:flex-no-wrap">
            <h1 class="text-xl">Настройка доставки "{{ $deliveryType->name }}"</h1>
        </div>
        <div>
            <div class="p-2">

                @foreach($deliveryType->getTarif as $tarif)
                <div class="flex mt-2">
                    <span class="w-1/3 text-sm self-center">{{ $tarif->value }}</span>
                    <div class="w-2/3">
                        <input type="text" name="price[{{ $tarif->id }}]"
                               value="{{ !empty(old('price')) ? old('price') : $tarif->price }}"
            ...
            <div class="flex justify-between flex-wrap sm:flex-no-wrap">
                ...
                <a href="{{ route('admin.settings.deliveryTypes') }}"
